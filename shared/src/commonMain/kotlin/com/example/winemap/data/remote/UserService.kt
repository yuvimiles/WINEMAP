...
class UserService {
    ...
    suspend fun signUp(email: String, password: String, username: String): Result<User> {
        return try {
            println("DEBUG UserService: Starting signUp with email: $email")

            val authResult = auth.createUserWithEmailAndPassword(email, password)
            val firebaseUser = authResult.user
            println("DEBUG UserService: Firebase auth result: ${firebaseUser?.uid}")
            ...
            if (firebaseUser != null) {
                ...
                println("DEBUG UserService: Created user object: $user")

                try {
                    // Save user to Firestore
                    println("DEBUG UserService: Attempting to save to Firestore...")
                    firestore.collection(Constants.USERS_COLLECTION)
                        .document(firebaseUser.uid)
                        .set(user)

                    println("DEBUG UserService: Successfully saved to Firestore")
                    Result.Success(user)
                } catch (firestoreException: Exception) {
                    println("DEBUG UserService: Firestore save failed: ${firestoreException.message}")
                    // אפילו אם Firestore נכשל, המשתמש נרשם בהצלחה ב-Firebase Auth
                    Result.Success(user)
                }
            } else {
                println("DEBUG UserService: Firebase user is null")
                Result.Error(ApiException.AuthException)
            }
        } catch (e: Exception) {
            println("DEBUG UserService: SignUp failed with exception: ${e.message}")
            e.printStackTrace()
            Result.Error(ApiException.AuthException)
        }
    }
